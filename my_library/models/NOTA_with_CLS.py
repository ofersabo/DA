...
@Model.register('nota_with_cls')
class BertCLSEmbeddingsMTB(Model):
    def __init__(self,
                 vocab: Vocabulary,
        ...
        self.first_liner_layer = torch.nn.Linear(self.bert_type_model['hidden_size']*3,self.bert_type_model['hidden_size']*3)
        self.second_liner_layer = torch.nn.Linear(self.bert_type_model['hidden_size']*3,self.bert_type_model['hidden_size']*3)
        ...
        self.no_relation_vector = torch.randn([1,self.bert_type_model['hidden_size']*3],device=self.device,requires_grad=False)
    ...
    def extract_vectors_from_markers(self, embbeds, location):
        ...
        value = torch.cat((value, stacked_embed[:, 0, :]), dim=1)
